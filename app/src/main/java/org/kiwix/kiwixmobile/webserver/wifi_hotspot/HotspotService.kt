...
class HotspotService :
  ...
  internal suspend fun resyncServerWithRemainingBooks() {
    ...
    val currentBooks = dataSource.getLanguageCategorizedBooks().first()
      .filterIsInstance<BookOnDisk>()
    val existingPaths = currentBooks.map { it.zimReaderSource.toDatabase() }.toSet()
    ...
    // Keep the persisted selection in sync with the auto-resync, otherwise a book
    // dropped here stays marked "hosted" in kiwixDataStore until the user manually
    // re-toggles a selection - stale enough that re-adding the same file later could
    // get it silently re-hosted without the user having reselected it.
    val remainingBookIds = currentBooks
      .filter { it.zimReaderSource.toDatabase() in remainingPaths }
      .map { it.book.id }
      .toSet()
    kiwixDataStore?.setHostedBookIds(remainingBookIds)
