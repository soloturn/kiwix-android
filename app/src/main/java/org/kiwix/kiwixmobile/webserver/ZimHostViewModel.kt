...
class ZimHostViewModel @Inject constructor(
  ...
  private val hotspotServiceCommander: HotspotServiceCommander
   ...
   *
   * Goes through [hotspotServiceCommander] rather than [sendEvent]/[_events]: this fires
   * while the user is on the *Library* screen (that's where a ZIM gets deleted from), i.e.
   * exactly when nothing is collecting `events` (see `ZimHostRoute`'s
   * `repeatOnLifecycle(STARTED)`), so an emission here would be silently dropped and the
   * running server would keep serving the deleted book (#5081 review round 2).
   */
  private suspend fun handleBookRemoved(removedBookId: String) {
    ...
    if (paths.isEmpty()) {
      hotspotServiceCommander.stopServer()
    } else {
      hotspotServiceCommander.startServer(paths, restart = true)
    }
  }
